...
const RecapView: React.FC<RecapViewProps> = ({ conversation }) => {
  ...
  const renderContent = () => {
    ...
    switch (status) {
      ...
      case 'completed':
        if (recap && recap.trim() !== '') {
          return (
            <ScrollArea className="absolute inset-0 bg-gray-50 rounded-md p-4 prose max-w-none">
                <ReactMarkdown
                  components={{
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 w-full h-full flex flex-col">
      ...
      {/* Main Content Area - This div will grow and allow ScrollArea within renderContent to manage scroll */}
